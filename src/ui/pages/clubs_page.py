...
from src.ui.components.center_card_component import center_card_component
from src.ui.components.club_card_component import club_card_component
from src.ui.components.header_component.advanced_search_header import advanced_search_clubs_header_component
from src.ui.components.list_control_component import list_control_component
from src.ui.components.pagination_component import pagination_component
from src.ui.components.search_sider_component import search_sider_component
...
class ClubsPage(BasePage):
    ...
    def search_clubs_header(self):
        if not self._search_clubs_header:
            self._search_clubs_header = advanced_search_clubs_header_component(self.driver, self._driver.find_element(*SEARCH_CLUBS_HEADER_WEB_ELEMENT))
        return self._search_clubs_header

    @property
    def pagination(self):
        if not self._pagination:
            self._pagination = pagination_component(self._driver, self._driver.find_element(*PAGINATION_WEB_ELEMENT))
        return self._pagination

    @property
    def search_sider(self):
        if not self._search_sider:
            self._search_sider = search_sider_component(self._driver.find_element(*SEARCH_SIDER_WEB_ELEMENT))
        return self._search_sider

    @property
    def list_control(self):
        if not self._list_control:
            self._list_control = list_control_component(self._driver.find_element(*LIST_CONTROL_WEB_ELEMENT))
    ...
    def club_card_list(self):
        if not self._club_card_list:
            ...
            for club in club_elements_list:
                self._club_card_list.append(club_card_component(club))
    ...
    def center_card_list(self):
        if not self._center_card_list:
            ...
            for center in center_elements_list:
                self._center_card_list.append(center_card_component(center))
